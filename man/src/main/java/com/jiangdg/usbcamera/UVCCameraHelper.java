...
import com.jiangdg.usbcamera.utils.HelperCamera;
...
public class UVCCameraHelper {
    ...
    public void requestPermission(int index) {
        ...
        List<UsbDevice> devList2 = new ArrayList<>();

        for(UsbDevice dev: devList) {
            if(dev.getVendorId() != 4292) {
                devList2.add(dev);
            }
        }

        int count = devList2.size();
        if (index >= count)
            new IllegalArgumentException("index illegal,should be < devList.size()");
        if (mUSBMonitor != null) {
            try {
                mUSBMonitor.requestPermission(devList2.get(index));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
...
//            throw new NullPointerException("mUSBMonitor ="+mUSBMonitor+"deviceFilters=;"+deviceFilters);
        ...
        return mUSBMonitor.getDeviceList(HelperCamera.INSTANCE.getCameraUSBFilters(deviceFilters));
    }
